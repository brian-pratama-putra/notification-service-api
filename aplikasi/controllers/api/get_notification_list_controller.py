...
from aplikasi.dao.api.db_dao import get_notification_list
from aplikasi.dao.api.redis_dao import cache_get


async def get_notification_list_controller(request):
    data = await request.json()

    v_method        = data.get("method", "")
    v_is_read       = data.get("is_read", None)
    v_channel       = data.get("channel", None)
    ...
    v_hasil     = await get_notification_list(v_user_id, v_is_read, v_channel)
