...
from aplikasi.dao.api.db_dao import mark_all_notifications_read
from aplikasi.dao.api.redis_dao import cache_get, cache_delete
import asyncio


async def mark_all_read_controller(request):
    ...
    v_hasil     = await asyncio.to_thread(mark_all_notifications_read, v_user_id)

    if v_hasil["status"] == "T":
        await cache_delete(f"notif:list:{v_user_id}:all:all")
        await cache_delete(f"notif:summary:{v_user_id}")
        await cache_delete(f"notif:unread:{v_user_id}")
