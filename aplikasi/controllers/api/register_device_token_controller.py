...
from aplikasi.dao.api.db_dao import upsert_device_token
from aplikasi.dao.api.redis_dao import cache_get
import asyncio


async def register_device_token_controller(request):
    data = await request.json()

    v_method        = data.get("method", "")
    v_device_token  = data.get("device_token", "")
    v_platform      = data.get("platform", "")
    v_session_key   = data.get("session_key", "")
    v_datetime      = data.get("datetime", "")
    v_checksum      = data.get("checksum", "")

    if not all([v_device_token, v_platform, v_session_key, v_datetime, v_checksum]):
        return await response_service(request, v_method, 422, 400, "Invalid Request Data")

    if v_platform not in ("android", "ios"):
        return await response_service(request, v_method, 422, 400, "Platform harus android atau ios")

    v_app_payload   = f"{v_method}#{v_device_token}#{v_platform}#{v_datetime}#{secret_key_request}"
    ...
    v_hasil     = await asyncio.to_thread(upsert_device_token, v_user_id, v_device_token, v_platform)
