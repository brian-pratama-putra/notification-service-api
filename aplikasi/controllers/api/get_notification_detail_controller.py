...
from aplikasi.dao.api.db_dao import get_notification_detail
from aplikasi.dao.api.redis_dao import cache_get
import asyncio


async def get_notification_detail_controller(request):
    ...
    v_hasil     = await asyncio.to_thread(get_notification_detail, v_notification_id, v_user_id)

    if v_hasil["status"] == "T" and v_hasil["result"]:
        return await response_service(request, v_method, 200, 200, "Success", {"data": v_hasil["result"][0]})
