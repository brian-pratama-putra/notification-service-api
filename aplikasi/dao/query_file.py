...
APP_NAME    = f"notification-service-{INSTANCE_ID}"
...
class QueryStringDb(object):
    ...
    def execute_many(self, p_query, p_params: list, p_response="Execute many commit successfully!"):
        try:
            self._db_connection.get_cursor().executemany(p_query, p_params)
            self._db_connection.commit()
            return response_json(200, "T", p_response, None)
        except psycopg2.Error as error:
            self._db_connection.rollback()
            v_msg = f"Error execute many: {error.pgcode}, {str(error.pgerror)}"
            return response_json(400, "F", v_msg, None)
